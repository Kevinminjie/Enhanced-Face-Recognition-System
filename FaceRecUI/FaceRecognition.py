...
class Face_MainWindow(Ui_MainWindow):
        ...
        try:
            ...
            if os.path.exists("../data/features_all.csv"):
                ...
                for encoding in encodings:
                    try:
                        csv_rd = pd.read_csv(path_features_known_csv, header=None, encoding=encoding)
                        break
                    except UnicodeDecodeError:
                        continue
                    except Exception as e:
                        print(f"读取CSV文件时发生错误 (编码: {encoding}): {str(e)}")
                        continue
                
                if csv_rd is None:
                    print("错误：无法使用任何编码读取CSV文件")
                    return 0
                ...
                for i in range(csv_rd.shape[0]):
                    ...
                    for j in range(1, 129):
                        if j < csv_rd.shape[1]:
                            feature_val = csv_rd.iloc[i][j]
                            if pd.isna(feature_val) or feature_val == '':
                                features_someone_arr.append(0.0)
                            else:
                                try:
                                    features_someone_arr.append(float(feature_val))
                    ...
                    # 读取姓名（第0列）
                    if csv_rd.shape[1] > 0:
                        name_val = csv_rd.iloc[i][0]
                        if pd.isna(name_val) or name_val == '':
                            self.face_name_exist.append("未知人脸")
                        else:
                            self.face_name_exist.append(str(name_val))
                    else:
                        self.face_name_exist.append("未知人脸")
    ...
    def get_img_doing(self):
        ...
        try:
            ...
            try:
                ...
                print(f"更新界面时发生异常: {str(e)}")
                
        except Exception as e:
            self.label_loadface.setText(f"保存人脸时发生异常：{str(e)}")
            print(f"get_img_doing异常: {str(e)}")
        ...
        try:
            ...
            if face_name != "请在此输入人脸名" and face_name != "":
                ...
                if not os.path.isdir(face_dir_path):
                    try:
                        os.makedirs(face_dir_path, exist_ok=True)  # 不存在则创建新文件夹
                        # 在界面中提示已经新建
                    ...
                    try:
                        for dir_path, listdir, files in os.walk(face_dir_path):
                            for file in files:
                    ...
                    if len(faces) != 0:
                        ...
                        for k, d in enumerate(faces):
                            try:
                                ...
                                # 剪切出人脸
                                crop_face = img_rd[x1: x2, y1: y2]
                                if crop_face.size > 0:  # 确保裁剪的人脸不为空
                                    self.current_face = crop_face
                                    self.disp_face(crop_face)
    ...
    def cv_imread(filePath):
        ...
        try:
            ...
            print(f"读取图像文件异常: {filePath}, 错误: {str(e)}")
            return None
